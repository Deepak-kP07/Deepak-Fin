...
import { getMoneyProfile, updateMoneyProfile, deleteMoneyProfile } from '@/lib/server/moneyProfileCrud'

export async function GET(request, { params }) {
  const { id } = await params
  const { supabase, user, cors, response } = await requireUser(request)
  if (response) return response
  const row = await getMoneyProfile(supabase, user, id)
...
export async function PATCH(request, { params }) {
  ...
  const result = await updateMoneyProfile(supabase, user, id, body)
...
export async function DELETE(request, { params }) {
  ...
  const { supabase, user, cors, response } = await requireUser(request)
  if (response) return response
  const result = await deleteMoneyProfile(supabase, user, id)
  if (result.error) return cors(NextResponse.json({ error: result.error.message }, { status: result.error.status || 400 }))
