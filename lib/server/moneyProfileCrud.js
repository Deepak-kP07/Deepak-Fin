...
export async function createMoneyProfileEntry(supabase, user, body) {
  ...
  // profile's own link when unset, same COALESCE the mirror function itself applies.
  if (profile.linked_account_id || payload.account_id) {
...
export async function updateMoneyProfileEntry(supabase, user, id, body) {
  ...
  // now creates one on the fly if an account_id/linked_account_id applies (e.g. this entry didn't
  // have an applicable account when first created, and one was just set on edit).
  if (updated?.linked_transaction_id || profile?.linked_account_id || updated?.account_id) {
