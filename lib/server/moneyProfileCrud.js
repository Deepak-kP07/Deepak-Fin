...
// A profile's opening_balance is money already held for someone as of a point in time — unlike
// every entry logged afterward, it's never itself mirrored through money_profile_entries (that
// would double it in profileTotals, since opening_balance already counts toward the profile's own
// balance there). Left unmirrored entirely, a freshly-linked profile with a nonzero opening_balance
// never showed up in the linked account's own balance until an actual entry was logged — reads as
// "linking did nothing" for anyone whose starting point IS the money that should count. Mirrors it
// the same way an entry does (a plain income/expense transaction on the linked account), tracked
// on the profile row itself (opening_balance_linked_transaction_id) instead. Idempotent and safe
// to call on every create/update, including a no-op save — reverses/updates/creates as needed, so
// an already-affected profile self-heals the same way updateMoneyProfileEntry's mirror call does.
async function syncOpeningBalanceMirror(supabase, profile) {
  const amount = Number(profile.opening_balance || 0)
  const applies = !!profile.linked_account_id && amount !== 0
  const existingTxId = profile.opening_balance_linked_transaction_id

  if (!applies) {
    if (existingTxId) {
      await supabase.from('transactions').delete().eq('id', existingTxId)
      await supabase.from('money_profiles').update({ opening_balance_linked_transaction_id: null }).eq('id', profile.id)
    }
    return
  }

  const direction = amount > 0 ? 'income' : 'expense'
  const category_id = await ensureCategory(supabase, profile.user_id, 'Balance adjustment', direction)
  const fields = {
    account_id: profile.linked_account_id, amount: Math.abs(amount), type: direction,
    description: `Opening balance — ${profile.name}`, date: profile.opening_balance_date,
    category_id, linked_module: 'money_profile_opening', linked_module_id: profile.id,
  }

  if (existingTxId) {
    await supabase.from('transactions').update(fields).eq('id', existingTxId)
  } else {
    const { data: tx } = await supabase.from('transactions').insert({ user_id: profile.user_id, ...fields }).select().single()
    if (tx) await supabase.from('money_profiles').update({ opening_balance_linked_transaction_id: tx.id }).eq('id', profile.id)
  }
}
...
export async function createMoneyProfile(supabase, user, body) {
  ...
  await syncOpeningBalanceMirror(supabase, created)
  const { data: refreshed } = await supabase.from('money_profiles').select('*').eq('id', created.id).maybeSingle()
  return { created: { ...(refreshed || created), my_role: 'owner' } }
}
...
export async function updateMoneyProfile(supabase, user, id, body) {
  ...
  if (error) return { error }
  if (updated) await syncOpeningBalanceMirror(supabase, updated)
  const { data: refreshed } = await supabase.from('money_profiles').select('*').eq('id', id).maybeSingle()
  return { updated: refreshed ? { ...refreshed, my_role: role } : null }
}

export async function deleteMoneyProfile(supabase, user, id) {
  const { data: profile } = await supabase.from('money_profiles').select('user_id, opening_balance_linked_transaction_id').eq('id', id).maybeSingle()
  ...
  // mirror_money_profile_entry_transaction's delete branch does for a single entry. The opening-
  // balance mirror (if any) has the same orphan risk and joins the same batch delete.
  ...
  if (profile.opening_balance_linked_transaction_id) transactionIds.push(profile.opening_balance_linked_transaction_id)
...
export async function createMoneyProfileEntry(supabase, user, body) {
  ...
  // An entry can override which account it mirrors to (payload.account_id) — falls back to the
  // profile's own link when unset, same COALESCE the mirror function itself applies. A directly
  // picked credit card (payload.credit_card_id) applies regardless of any account fallback — its
  // own absence here used to mean a card-funded entry on a profile with no linked_account_id
  // never got mirrored at all (no transaction, no card outstanding-balance bump), even though the
  // entry itself correctly stored credit_card_id.
  if (profile.linked_account_id || payload.account_id || payload.credit_card_id) {
...
export async function updateMoneyProfileEntry(supabase, user, id, body) {
  ...
  const { data: profile } = await supabase.from('money_profiles').select('user_id, linked_account_id').eq('id', existing.profile_id).maybeSingle()
  ...
  if (error) return { error }
  // Call the mirror even when there's no existing linked_transaction_id yet — the function itself
  // now creates one on the fly if an account_id/linked_account_id/credit_card_id applies (e.g.
  // this entry didn't have an applicable funding source when first created, and one was just set
  // on edit). credit_card_id must be checked here too, same reasoning as createMoneyProfileEntry.
  if (updated?.linked_transaction_id || profile?.linked_account_id || updated?.account_id || updated?.credit_card_id) {
    ...
    if (!updated.linked_transaction_id) {
      const { data: refreshed } = await supabase.from('money_profile_entries').select('*').eq('id', id).maybeSingle()
      return { updated: refreshed || updated }
    }
  }
  return { updated }
}
