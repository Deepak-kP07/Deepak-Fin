...
export async function deleteMoneyProfile(supabase, user, id) {
  ...
  // Deleting the whole profile leaves any mirrored transactions from its entries as-is (they
  // cascade-delete at the DB level along with the entries, but a mirrored transaction has no FK
  // back to the entry, only linked_module_id) — a pre-existing, disclosed limitation carried
  // over unchanged from the single-owner era, not something this feature introduces or fixes.
...
export async function deleteMoneyProfileEntry(supabase, user, id) {
  ...
  if (existing.linked_transaction_id) {
    await supabase.rpc('mirror_money_profile_entry_transaction', { p_entry_id: id, p_action: 'delete' })
  }
