...
export async function approvePendingTransaction(supabase, userId, id, overrides = {}) {
  ...
  // Neither is ever set by the SMS parsing/ingestion path (lib/sms/parseEngine.js doesn't
  // extract a date/time from the message body) — the pending row's date/time columns stay null
  // right up to approval, where "now" is the closest available stand-in. For auto-approved
  // senders this runs synchronously right after ingestion, so it's effectively the real
  // detection time; for a manually-reviewed one it's the review time instead, same as leaving
  // a manually-entered transaction's date/time at today/now by default.
  const now = new Date()
  const txBody = {
    amount: merged.amount,
    type: merged.type,
    description: merged.description || merged.merchant || 'SMS transaction',
    date: merged.date || now.toISOString().slice(0, 10),
    time: merged.time || now.toTimeString().slice(0, 5),
