...
async function freshKiteToken(supabase, userId) {
  const { data: profile } = await supabase.from('profiles').select('kite_access_token,kite_access_token_at').eq('id', userId).maybeSingle()
  return isKiteTokenFresh(profile?.kite_access_token, profile?.kite_access_token_at) ? profile.kite_access_token : null
}

export async function syncPortfolioFromKite(supabase, userId, portfolioId) {
  const accessToken = await freshKiteToken(supabase, userId)
  if (!accessToken) return { error: { message: 'Kite isn’t connected (or the token has gone stale) — reconnect from Investments first.' } }
  ...
  for (const { id, ...patch } of toUpdate) await supabase.from('holdings').update(patch).eq('id', id).eq('user_id', userId)
...
export async function syncMutualFundsFromKite(supabase, userId) {
  ...
  try {
    ...
    for (const { id, ...patch } of toUpdate) await supabase.from('sips').update(patch).eq('id', id).eq('user_id', userId)
