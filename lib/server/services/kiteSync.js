...
// alone can't tell them apart from the outside. Logged server-side only — the client gets a
// clean, actionable message plus `code: 'kite_token_stale'` so the UI can offer a one-click
// reconnect instead of just telling the user to go find it themselves.
function staleTokenError(rawTokenAt, hasSavedToken) {
  console.warn(hasSavedToken ? `Kite token stale: saved at ${rawTokenAt}, now ${new Date().toISOString()}` : 'Kite token stale: no token saved on this profile')
  return { message: 'Your Kite session has expired for the day — reconnect to sync again.', code: 'kite_token_stale' }
}
...
export async function syncPortfolioFromKite(supabase, userId, portfolioId) {
  ...
  if (!accessToken) return { error: staleTokenError(rawTokenAt, hasSavedToken) }
  ...
  // `available.cash` is only the settled-ledger cash component — same-day UPI/bank top-ups sit
  // in `intraday_payin` until end-of-day settlement, so cash alone reads far lower than what
  // Zerodha's own dashboard shows as "Margin available." `net` (mirrored by available.live_balance)
  // is what that figure actually is: net usable funds, exactly matching Kite's own UI.
  const cashUpdate = { last_kite_sync_at: new Date().toISOString() }
  const availableMargin = margins?.net ?? margins?.available?.live_balance
  if (availableMargin != null) cashUpdate.cash_balance = Number(availableMargin)
