...
export async function createTransaction(supabase, userId, body) {
  ...
  // A repayment logged against a lend/borrow record (the TransactionForm's "repayment" quick-
  // picker, see app/page.js) has no natural category to pick from — reuses the same "Loan / Debt"
  // default category ensureCategory already applies to "Lend more"/"Borrow more"
  // (lib/server/lendBorrowCrud.js's addToLendBorrow) instead of landing Uncategorised. Never
  // overrides a category the user actually picked themselves.
  if (body.linked_module === 'lend' && body.linked_module_id && !body.category_id) {
    body = { ...body, category_id: await ensureCategory(supabase, userId, 'Loan / Debt', body.type === 'income' ? 'income' : 'expense') }
  }
