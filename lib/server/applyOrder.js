export function applyOrder(query, table) {
  ...
  if (table === 'lend_borrow' || table === 'lend_repayments' || table === 'lend_borrow_additions' || table === 'money_profile_entries') return query.order('date', { ascending: false })
  if (table === 'kite_orders') return query.order('order_timestamp', { ascending: false, nullsFirst: false })
  if (table === 'loan_payments' || table === 'scholarship_payments' || table === 'chit_fund_payments') return query.order('payment_date', { ascending: false })
  ...
  if (table === 'recurring_transactions' || table === 'recurring_money_profile_entries') return query.order('next_due_date', { ascending: true })
