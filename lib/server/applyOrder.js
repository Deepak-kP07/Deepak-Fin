export function applyOrder(query, table) {
  ...
  if (table === 'accounts' || table === 'portfolios' || table === 'holdings' || table === 'credit_cards' || table === 'other_investments' || table === 'money_profiles') return query.order('created_at', { ascending: true })
