...
export async function updateLendBorrow(supabase, user, id, body) {
  const { data: record } = await supabase.from('lend_borrow').select('user_id, amount, amount_repaid').eq('id', id).maybeSingle()
  ...
  // Recompute status against whatever amount comes out of this edit (same formula as
  // lendRepayment.js/lendAddition.js) — otherwise raising the amount on an already-fully-repaid
  // record (e.g. fixing a typo after it hit 'returned') left the stale 'returned' badge in place
  // even though amount_repaid no longer covers the corrected amount.
  const repaid = Number(record.amount_repaid || 0)
  const amount = Number(patch.amount ?? record.amount)
  patch.status = repaid >= amount ? 'returned' : repaid > 0 ? 'partial' : 'pending'
...
// Same shape as listLendRepayments — role-aware read side for /finance/summary. Additions have
// no dedicated write route either for the bank/cash/no-account case (that goes through the
// transactions catch-all's applyLendAddition/reverseLendAddition, same trigger-based pattern as
// repayments) — only addToLendBorrow below, which handles the one case that can't: a
// credit-card-funded addition, mirroring how card-funded creation is handled directly in
// genericCrud.js rather than through a linked_module hook.
export async function listLendAdditions(supabase) {
  const { data } = await applyOrder(supabase.from('lend_borrow_additions').select('*'), 'lend_borrow_additions')
  return data || []
}

// "+ Lend more" / "+ Borrow more" — logs that additional money moved against an existing
// lend/borrow record instead of creating a brand-new one. Owner-only, same tier as logging a
// repayment (lib/lendBorrowSharing.js's canLogRepayment) — this is the identical class of
// side-effecting write (mirrors a transaction, can bump a credit card's outstanding balance).
export async function addToLendBorrow(supabase, user, id, body) {
  const { data: record } = await supabase.from('lend_borrow').select('*').eq('id', id).maybeSingle()
  if (!record) return { error: { message: 'Record not found', status: 404 } }
  const role = await roleFor(supabase, record.user_id, id, user.id)
  if (role !== 'owner') return { error: { message: 'Only the owner can log this', status: 403 } }
  ...
  const cardId = typeof rawAccountId === 'string' && rawAccountId.startsWith('cc:') ? rawAccountId.slice(3) : null
  if (cardId && !isLent) return { error: { message: 'A credit card can only fund lending more, not borrowing more', status: 400 } }
  const accountId = cardId ? null : (rawAccountId || null)

  // No account chosen at all skips this whole block — same "skip account impact" option the
  // original record's own creation form offers.
  let txId = null
  if (accountId || cardId) {
    const categoryId = isLent
      ? await ensureCategory(supabase, record.user_id, 'Lended', 'expense')
      : await ensureCategory(supabase, record.user_id, 'Loan / Debt', 'income')
    const nowStr = new Date().toTimeString().slice(0, 5)
    const txPayload = {
      user_id: record.user_id, account_id: accountId, amount, type: isLent ? 'expense' : 'income',
      description: `${isLent ? 'Lent' : 'Borrowed'} more ${isLent ? 'to' : 'from'} ${record.person_name}`,
      date, time: nowStr, category_id: categoryId,
      // Card-funded uses 'credit_card' (never 'lend_addition') so it hits the existing generic
      // outstanding-adjustment path instead of applyLendAddition's — exactly how card-funded
      // origination is tagged in genericCrud.js.
      linked_module: cardId ? 'credit_card' : 'lend_addition', linked_module_id: cardId || id,
      notes: body.notes || null,
    }
    const { data: tx, error: txError } = await supabase.from('transactions').insert(txPayload).select().single()
    if (txError) return { error: txError }
    txId = tx.id
    if (cardId) {
      const { data: card } = await supabase.from('credit_cards').select('current_outstanding').eq('id', cardId).eq('user_id', record.user_id).maybeSingle()
      if (card) await supabase.from('credit_cards').update({ current_outstanding: Number(card.current_outstanding || 0) + amount }).eq('id', cardId).eq('user_id', record.user_id)
    }
  }

  await applyLendAddition(supabase, record.user_id, txId, id, amount, { date, account_id: accountId, notes: body.notes || null })
  const { data: updated } = await supabase.from('lend_borrow').select('*').eq('id', id).maybeSingle()
  return { updated: updated ? { ...updated, my_role: role } : null }
}
