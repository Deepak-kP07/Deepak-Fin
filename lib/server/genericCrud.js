...
export async function createInCollection(supabase, user, table, body) {
  ...
  if (table === 'pending_transactions' && created?.id) {
    const patch = {}
    if (created.last4_hint && !created.account_id && !created.credit_card_id) {
      ...
      if (account) patch.account_id = account.id
      else {
    ...
    if (created.matched_pattern_id && !created.suggested_category_id) {
      const { data: pattern } = await supabase.from('sms_parse_patterns').select('suggested_category_name, suggested_category_type').eq('id', created.matched_pattern_id).maybeSingle()
      if (pattern?.suggested_category_name && pattern?.suggested_category_type) {
        patch.suggested_category_id = await ensureCategory(supabase, user.id, pattern.suggested_category_name, pattern.suggested_category_type)
      }
    }
