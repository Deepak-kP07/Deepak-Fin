...
import { guessCategoryForMerchant } from '@/lib/sms/merchantCategories'
...
// Bank/UPI SMS often sign off with "-<Bank Name>" — used only by the generic SMS fallback
// (lib/sms/parseEngine.js) as a secondary account-resolution signal when the message carried no
// last4 at all (specific bank patterns already know their own bank_or_app label directly).
function extractBankNameSignature(rawMessage) {
  if (!rawMessage) return null
  const match = /-\s*([A-Z][A-Za-z.& ]{2,40}?)\s*$/.exec(rawMessage.trim())
  return match ? match[1].trim() : null
}

// Loose, bidirectional match so "HDFC" (what a user might type for their own account) and "HDFC
// Bank" (an SMS pattern's bank_or_app label) recognize each other, without requiring either side
// to spell the name out identically.
function bankNamesLooselyMatch(accountBankName, patternBankName) {
  if (!accountBankName || !patternBankName) return false
  const a = accountBankName.toLowerCase().trim()
  const b = patternBankName.toLowerCase().trim()
...
export async function createInCollection(supabase, user, table, body) {
  ...
  if (table === 'pending_transactions' && created?.id) {
    const { data: matchedPattern } = created.matched_pattern_id
      ? await supabase.from('sms_parse_patterns').select('is_generic, bank_or_app, suggested_category_name, suggested_category_type').eq('id', created.matched_pattern_id).maybeSingle()
      : { data: null }
    ...
    // last4 either wasn't in the message at all, or didn't match anything on file — try the
    // issuing bank's name next. Only trusted when it narrows to exactly one of the user's own
    // accounts; with two accounts at the same bank (or none), a name alone can't safely pick one,
    // so this stays unresolved rather than guessing wrong. Skipped for UPI-app patterns (Google
    // Pay, PhonePe, Paytm) — "the bank" there is whichever real account is linked to that app,
    // which the app's own name can't tell you.
    if (!resolvedAccount) {
      const bankName = matchedPattern?.is_generic ? extractBankNameSignature(created.raw_message) : matchedPattern?.bank_or_app
      if (bankName && !/\(upi\)/i.test(bankName)) {
        const { data: accountsList } = await supabase.from('accounts').select('id, bank_name').eq('user_id', user.id).not('bank_name', 'is', null)
        const matches = (accountsList || []).filter((a) => bankNamesLooselyMatch(a.bank_name, bankName))
        if (matches.length === 1) { patch.account_id = matches[0].id; resolvedAccount = true }
      }
    }
    ...
    // Category resolution, in order of confidence: (1) a merchant-name keyword match ("Zerodha"
    // → Investment, "Flipkart" → Shopping, etc. — lib/sms/merchantCategories.js) — this is checked
    // first since it's specific to THIS transaction, not just its bank/app; (2) the matched
    // pattern's own suggested_category_name, if it has one (currently none do — see the comment
    // on smsParsePatterns in db/schema.js for why bank/app alone rarely implies a category); (3) a
    // generic catch-all so an SMS-detected transaction is never left truly Uncategorised — a raw
    // UPI handle like "7989101143@pty" carries no real merchant name to guess from at all.
    if (!created.suggested_category_id) {
      const merchantGuess = guessCategoryForMerchant(created.merchant)
      if (merchantGuess) {
        patch.suggested_category_id = await ensureCategory(supabase, user.id, merchantGuess.name, merchantGuess.type)
      } else if (matchedPattern?.suggested_category_name && matchedPattern?.suggested_category_type) {
        ...
        const fallbackName = created.type === 'income' ? 'Other income' : 'Daily transactions'
        patch.suggested_category_id = await ensureCategory(supabase, user.id, fallbackName, created.type === 'income' ? 'income' : 'expense')
      }
    }
    ...
    // Instant, per-detection push — the daily digest cron (app/api/cron/notifications/route.js)
    // is only a once-a-day catch-up for whatever's still sitting unreviewed, not the primary
    // signal. Best-effort: a failed send here shouldn't fail the ingestion itself.
    sendPushToUser(supabase, user.id, {
      title: 'Transaction detected',
      body: `${enriched.merchant || (enriched.type === 'income' ? 'Money received' : 'Money spent')} — ${money(enriched.amount)} — waiting for approval`,
      url: `${process.env.NEXT_PUBLIC_BASE_URL || ''}/?view=pending`,
    }).catch(() => {})
