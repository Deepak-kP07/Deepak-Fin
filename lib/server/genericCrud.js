...
export async function createInCollection(supabase, user, table, body) {
  ...
  // A closed Family/Company profile blocks new entries (manual and bulk-import alike) until
  // reactivated — enforced here, not just hidden in the UI, since bulk import calls this same
  // path directly. Fetched once and reused below for the linked-transaction side effect.
  let moneyProfileForEntry = null
  if (table === 'money_profile_entries' && payload.profile_id) {
    const { data: profile } = await supabase.from('money_profiles').select('status, linked_account_id').eq('id', payload.profile_id).eq('user_id', user.id).maybeSingle()
    if (profile?.status === 'closed') return { error: { message: 'This profile is closed — reactivate it to add new entries.' } }
    moneyProfileForEntry = profile
  }
  ...
  // Side-effect: an entry under a bank-linked Family/Company profile mirrors into transactions
  // (capital counts as an inflow, same direction as income) — an unlinked profile's entries
  // never touch transactions at all, staying fully siloed.
  if (table === 'money_profile_entries' && created?.id) {
    const profile = moneyProfileForEntry
    if (profile?.linked_account_id) {
      const direction = payload.entry_type === 'expense' ? 'expense' : 'income'
      const categoryId = payload.category_id || await ensureCategory(supabase, user.id, 'Other', direction)
      const txPayload = { user_id: user.id, account_id: profile.linked_account_id, amount: Number(payload.amount), type: direction, description: payload.description, date: payload.date || new Date().toISOString().slice(0, 10), category_id: categoryId, linked_module: 'money_profile', linked_module_id: created.id, notes: payload.notes || null }
      const { data: tx } = await supabase.from('transactions').insert(txPayload).select().single()
      if (tx?.id) await supabase.from('money_profile_entries').update({ linked_transaction_id: tx.id }).eq('id', created.id).eq('user_id', user.id)
    }
  }
...
export async function updateInCollection(supabase, user, table, id, body) {
  ...
  // Keep a Family/Company entry's mirrored transaction in sync on edit — unlike lend_borrow/
  // scholarships (which never mirror edits), this module's "full accounting" bar means the two
  // books can't be allowed to drift apart.
  if (table === 'money_profile_entries' && updated?.linked_transaction_id) {
    const direction = updated.entry_type === 'expense' ? 'expense' : 'income'
    const categoryId = updated.category_id || await ensureCategory(supabase, user.id, 'Other', direction)
    await supabase.from('transactions').update({
      amount: Number(updated.amount), type: direction, description: updated.description, date: updated.date, category_id: categoryId, notes: updated.notes || null,
...
export async function deleteFromCollection(supabase, user, table, id) {
  ...
  // Deleting a Family/Company entry removes its mirrored transaction too (account balance
  // reverts automatically via the sync_account_balance trigger).
  if (table === 'money_profile_entries') {
    const { data: entry } = await supabase.from('money_profile_entries').select('linked_transaction_id').eq('id', id).eq('user_id', user.id).maybeSingle()
    if (entry?.linked_transaction_id) await supabase.from('transactions').delete().eq('id', entry.linked_transaction_id).eq('user_id', user.id)
  }
