...
// Single utilisation-severity scale, used everywhere a card's (or the whole fleet's) utilisation
// is shown — previously drifted into two different threshold sets (CreditCardsView's hero used
// <=30/<=60, CreditCardFlip's own card face used <50/<80), so the same number could read "High"
// on one component and merely "Rising" on another. 30%/60% follow the commonly-cited
// stay-under-30%-for-healthy-credit guideline.
export function utilisationSeverity(util) {
  if (util <= 30) return { label: 'Healthy', tone: 'text-emerald-300 light:text-emerald-700', bar: 'bg-emerald-400' }
  if (util <= 60) return { label: 'Rising', tone: 'text-amber-300 light:text-amber-700', bar: 'bg-amber-400' }
  return { label: 'High', tone: 'text-rose-300 light:text-rose-700', bar: 'bg-rose-400' }
}
...
// The window of spends that will land on the NEXT bill — i.e. from the most recent billing date
// up to (but not including) the next one. Distinct from nextBillDue()'s "billing" value, which
// rolls forward on the DUE date instead so "due in N days" stays correct through the whole
// payment window — this rolls forward on the billing date itself, since new spending always
// resets the moment a fresh statement is cut, regardless of whether the old one's been paid yet.
export function currentSpendingCycle(card, referenceDate = new Date()) {
  const bd = Number(card.billing_date)
  let start = new Date(referenceDate.getFullYear(), referenceDate.getMonth(), bd)
  if (start > referenceDate) start = new Date(start.getFullYear(), start.getMonth() - 1, bd)
  const end = new Date(start.getFullYear(), start.getMonth() + 1, bd)
  return { start, end }
}
