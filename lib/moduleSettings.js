...
export const MODULE_KEYS = [
  ...
  'scholarships', 'budgets', 'bucket_list', 'insights', 'pending_sms', 'chit_funds',
]
...
export const DEFAULT_MODULE_SETTINGS = {
  ...
  family_company: { enabled: true, order: 4 },
  lend_borrow: { enabled: true, order: 5 },
  scholarships: { enabled: false, order: 6 },
  budgets: { enabled: true, order: 7 },
  bucket_list: { enabled: false, order: 8 },
  insights: { enabled: false, order: 9 },
  // Off by default — this module doubles as the SMS auto-detect master switch (see
  // features/settings/SettingsSmsAutoDetect.jsx), and the feature needs a native Android wrapper
  // that most users won't have yet (see the plan's Phase E/F handoff notes).
  pending_sms: { enabled: false, order: 10 },
  // Off by default — a niche, opt-in savings scheme most users don't participate in, matching
  // scholarships/bucket_list/pending_sms rather than the always-on core modules.
  chit_funds: { enabled: false, order: 11 },
}
