...
export const ENTRY_TYPE_STYLE = {
  income: 'bg-emerald-400/15 text-emerald-200 light:text-emerald-700',
  capital: 'bg-accent-400/15 text-accent-200 light:text-accent-700',
  expense: 'bg-rose-400/15 text-rose-200 light:text-rose-700',
}
...
// A profile's balance is never stored — always opening_balance plus every qualifying entry since,
// computed fresh from whatever entries are in hand (mirrors how portfolio/other-investment totals
// are derived rather than persisted elsewhere in this app). "Qualifying" means dated on/after
// opening_balance_date — that date is when opening_balance already captured the real total, so an
// entry backdated before it would double-count if it were summed too. Entries before the cutoff
// still show up everywhere else (the list, category breakdowns) — they're just excluded here,
// letting someone log old entries purely for their own records without skewing the live balance.
export function profileTotals(profile, entries) {
  const cutoff = profile.opening_balance_date
  const counted = cutoff ? entries.filter((e) => e.date >= cutoff) : entries
  const income = counted.reduce((s, e) => s + (e.entry_type === 'income' ? Number(e.amount) : 0), 0)
  const capital = counted.reduce((s, e) => s + (e.entry_type === 'capital' ? Number(e.amount) : 0), 0)
  const expense = counted.reduce((s, e) => s + (e.entry_type === 'expense' ? Number(e.amount) : 0), 0)
