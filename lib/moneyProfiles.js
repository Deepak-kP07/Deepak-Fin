...
// A profile's balance is never stored — always opening_balance plus every entry logged against
// it, computed fresh from whatever entries are in hand (mirrors how portfolio/other-investment
// totals are derived rather than persisted elsewhere in this app). Every entry counts toward the
// balance regardless of its date — a date-based cutoff against opening_balance_date used to
// silently exclude entries dated before it (e.g. historical entries added via bulk import after
// the profile was set up), which made the balance quietly diverge from "everything logged here."
