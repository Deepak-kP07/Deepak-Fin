...
function renderReportBodyHtml(report) {
  ...
  const rows = [
    reportStatRow('Net worth', money(nw.now), `<span style="color:${changeColor};">${nw.change >= 0 ? '+' : '−'}${money(Math.abs(nw.change)).replace('-', '')}</span> this ${report.period === 'weekly' ? 'week' : 'month'}`),
    reportStatRow('Income', money(ie.income), ie.prevIncome ? `${money(ie.prevIncome)} last ${report.period === 'weekly' ? 'week' : 'month'}` : ''),
    reportStatRow('Expense', money(ie.expense), ie.prevExpense ? `${money(ie.prevExpense)} last ${report.period === 'weekly' ? 'week' : 'month'}` : ''),
  ...
  const categoriesHtml = report.topCategories.length
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">${report.topCategories.map((c) => `
      ...
      `).join('')}</table>`
    : `<p style="margin:8px 0 0;color:rgba(241,245,249,0.45);font-size:13px;">No spending this ${report.period === 'weekly' ? 'week' : 'month'}.</p>`
  const upcoming = [
