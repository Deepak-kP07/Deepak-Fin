...
function reportStatRow(label, valueHtml, subHtml = '') {
  return `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="color:rgba(241,245,249,0.6);font-size:13px;">${label}${subHtml ? `<br><span style="color:rgba(241,245,249,0.4);font-size:12px;">${subHtml}</span>` : ''}</td>
            <td align="right" style="color:#f1f5f9;font-size:15px;font-weight:600;white-space:nowrap;">${valueHtml}</td>
          </tr>
        </table>
      </td>
    </tr>
...
function renderReportBodyHtml(report) {
  ...
  const rows = [
    reportStatRow('Net worth', money(nw.now), `<span style="color:${changeColor};">${nw.change >= 0 ? '+' : '−'}${money(Math.abs(nw.change)).replace('-', '')}</span> this ${periodLabel}`),
    reportStatRow('Income', money(ie.income), changeSubLabel(ie.prevIncome, ie.incomeChangePct, report.period)),
    reportStatRow('Expense', money(ie.expense), changeSubLabel(ie.prevExpense, ie.expenseChangePct, report.period)),
    reportStatRow('Savings rate', `${ie.savingsRatePct}%`, ie.net >= 0 ? `${money(ie.net)} net` : `${money(Math.abs(ie.net))} short`),
  ]
  if (report.avgExpense !== null) {
    rows.push(reportStatRow('Transactions', String(report.txnCount), `${money(report.avgExpense)} avg. expense`))
  }
  if (report.budget) {
    const b = report.budget
    const paceLabel = 'daysLeft' in b ? `${b.pct}% of this month's budget used · ${b.daysLeft} day${b.daysLeft === 1 ? '' : 's'} left` : `${b.pct}% used`
    rows.push(reportStatRow('Budget', money(b.spent) + ' of ' + money(b.budgeted), paceLabel))
  }
  const categoriesHtml = report.topCategories.length
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">${report.topCategories.map((c) => `
        <tr><td style="padding:4px 0;color:rgba(241,245,249,0.65);font-size:13px;">${escapeHtml(c.category?.name || 'Uncategorized')}</td>
        <td align="right" style="padding:4px 0;color:rgba(241,245,249,0.85);font-size:13px;">${money(c.amount)}</td></tr>
      `).join('')}</table>`
    : `<p style="margin:8px 0 0;color:rgba(241,245,249,0.45);font-size:13px;">No spending this ${periodLabel}.</p>`
  const biggestHtml = report.biggestTransactions.length
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">${report.biggestTransactions.map((t) => `
        <tr><td style="padding:4px 0;color:rgba(241,245,249,0.65);font-size:13px;">${escapeHtml(t.description || t.category?.name || 'Uncategorized')} <span style="color:rgba(241,245,249,0.35);">· ${shortDateLabel(t.date)}</span></td>
        <td align="right" style="padding:4px 0;color:rgba(241,245,249,0.85);font-size:13px;">${money(t.amount)}</td></tr>
      `).join('')}</table>`
    : `<p style="margin:8px 0 0;color:rgba(241,245,249,0.45);font-size:13px;">No spending this ${periodLabel}.</p>`
  const accountsHtml = report.accountActivity.length
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">${report.accountActivity.map((a) => `
        <tr><td style="padding:4px 0;color:rgba(241,245,249,0.65);font-size:13px;">${escapeHtml(a.account?.name || 'Unknown account')} <span style="color:rgba(241,245,249,0.35);">· ${a.count} txn${a.count === 1 ? '' : 's'}</span></td>
        <td align="right" style="padding:4px 0;color:rgba(241,245,249,0.85);font-size:13px;">${money(a.spent)}</td></tr>
      `).join('')}</table>`
    : `<p style="margin:8px 0 0;color:rgba(241,245,249,0.45);font-size:13px;">No spending this ${periodLabel}.</p>`
  const upcoming = [
    ...report.upcoming.cards.map(({ card, days }) => `${escapeHtml(card.name)} bill — ${days < 0 ? `overdue by ${Math.abs(days)}d` : days === 0 ? 'due today' : `due in ${days}d`} — ${money(card.current_outstanding)}`),
    ...report.upcoming.loans.map(({ loan, due }) => `${escapeHtml(loan.name)} EMI — due ${due} — ${money(loan.emi_amount)}`),
  ]
  const upcomingHtml = upcoming.length
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">${upcoming.map((line) => `<tr><td style="padding:4px 0;color:rgba(241,245,249,0.65);font-size:13px;">${line}</td></tr>`).join('')}</table>`
    : `<p style="margin:8px 0 0;color:rgba(241,245,249,0.45);font-size:13px;">Nothing due soon.</p>`

  return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${rows.join('')}</table>
    <p style="margin:24px 0 0;color:#f1f5f9;font-size:14px;font-weight:600;">Top spending categories</p>
    ${categoriesHtml}
    <p style="margin:24px 0 0;color:#f1f5f9;font-size:14px;font-weight:600;">Biggest transactions</p>
    ${biggestHtml}
    <p style="margin:24px 0 0;color:#f1f5f9;font-size:14px;font-weight:600;">By account</p>
    ${accountsHtml}
    <p style="margin:24px 0 0;color:#f1f5f9;font-size:14px;font-weight:600;">Upcoming</p>
    ${upcomingHtml}
