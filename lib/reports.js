...
// Largest individual expenses in the period — expense-only, same convention as topCategories()
// (a transfer moving money between your own accounts isn't "spend").
function biggestTransactions(transactions, categories, start, end, limit = 5) {
  return transactions
...
// How many transactions happened (income+expense, same transfer-exclusion as incomeExpenseFor())
// and how big a typical expense was — activity-volume context the raw income/expense totals alone
// don't convey. avgExpense is null (not Infinity/NaN) when there were no expenses to average.
function transactionStats(transactions, start, end) {
  let txnCount = 0, expenseCount = 0, expenseTotal = 0
  for (const t of transactions) {
    if (t.type === 'transfer' || !inRange(t.date, start, end)) continue
    txnCount += 1
    if (t.type === 'expense') { expenseCount += 1; expenseTotal += Number(t.amount || 0) }
  }
  return { txnCount, avgExpense: expenseCount > 0 ? expenseTotal / expenseCount : null }
}

// Which accounts saw the most expense activity in the period — same expense-only, top-N shape as
// topCategories()/biggestTransactions(), just grouped by account instead.
function accountActivity(transactions, accounts, start, end, limit = 5) {
  const byAccount = {}
  for (const t of transactions) {
    if (t.type !== 'expense' || !inRange(t.date, start, end) || !t.account_id) continue
    const bucket = byAccount[t.account_id] || { spent: 0, count: 0 }
    bucket.spent += Number(t.amount || 0)
    bucket.count += 1
    byAccount[t.account_id] = bucket
  }
  return Object.entries(byAccount)
    .sort((a, b) => b[1].spent - a[1].spent)
    .slice(0, limit)
    .map(([accountId, { spent, count }]) => ({ account: accounts.find((a) => a.id === accountId) || null, spent, count }))
}

// null (not a misleading "+∞%"/"0%") when there's no prior-period figure to compare against.
...
export function buildFinancialReport({
  ...
  const expenseChangePct = pctChange(expense, prevExpense)
  ...
  return {
    ...
    incomeExpense: { income, expense, net, savingsRatePct, prevIncome, prevExpense, incomeChangePct, expenseChangePct },
    budget,
    topCategories: topCategories(transactions, categories, bounds.start, bounds.end),
    biggestTransactions: biggestTransactions(transactions, categories, bounds.start, bounds.end),
    accountActivity: accountActivity(transactions, accounts, bounds.start, bounds.end),
    ...transactionStats(transactions, bounds.start, bounds.end),
