...
const CASES = [
  ...
  {
    // Real observed format (not a guess) — see lib/sms/patterns.seed.js. This one was missed
    // entirely (parseSms returned null) before the pattern existed, since the generic fallback
    // hard-requires a last4 and Canara's masking here only ever reveals 3 digits.
    label: 'Canara Bank credit',
    sender: 'AD-CANBNK-S',
    body: 'Dear Customer, Acct XXX601 credited with INR 5.00 on 03/09/26 from PITCHIKA  KA; UPI:624651599163; Bal INR 384.10-CanaraBank',
    expect: { amount: '5.00', type: 'income', merchant: 'PITCHIKA  KA' },
  },
  {
  ...
  {
    // Real observed format (not a guess — a prior version of this test used "Debited for Rs:",
    // which was an unconfirmed guess that turned out wrong; real debit alerts say "Debited Rs:"
    // with no "for" — see patterns.seed.js). Also real: the "Fvg: <name>" clause naming who the
    // money went to, which the credit-side format below doesn't have.
    label: 'Union Bank of India debit',
    sender: 'AD-UNIONB',
    body: 'Union Bank of India A/c *3311 Debited Rs:410.00 on 01-09-2026 19:06:21 by Mob Bk ref no 624450441030, Fvg: RAMESH K Avl Bal Rs:6500.00. Not you?Call 18002333/SMS BLOCK 3311 to 8879365472',
    expect: { amount: '410.00', type: 'expense', last4_hint: '3311', merchant: 'RAMESH K' },
  },
  {
  ...
  {
    // Real observed format — see patterns.seed.js. Note lowercase "a/c" (unlike every other bank
    // seeded here) and the one shape the generic fallback can never handle on its own: it names
    // BOTH "debited" (self) and "credited to VPA X" (the counterparty) in the same sentence, which
    // parseGeneric's ambiguity guard reads as "both, therefore neither" and drops entirely.
    label: 'APGBank debit',
    sender: 'AD-APGBSM',
    body: 'Your a/c no. XXXXXXXXXXX7120 is debited for Rs.1174.50 on 06/09/2026 17:14:44 and credited to VPA repaymentsnesfb@slc (UPI Ref no 624975308139) -APGBank',
    expect: { amount: '1174.50', type: 'expense', last4_hint: '7120', merchant: 'repaymentsnesfb@slc' },
  },
]
...
describe('parseSms', () => {
  ...
  it('generic-matches a real APGBank credit (no dedicated pattern needed — "credited in your A/c" has no ambiguous second direction word)', () => {
    const result = parseSms(SMS_PATTERN_SEEDS, { sender: 'AD-APGBSM', body: 'Dear Customer, An amount of Rs.1484/- is credited in your A/c XXXX7120 on 31-08-2026, UPI Ref No:159203224962. At present A/c Balance is Rs.1899.25 -APGBank' })
    expect(result).not.toBeNull()
    expect(result.amount).toBe('1484')
    expect(result.type).toBe('income')
    expect(result.last4_hint).toBe('7120')
  })

  it('generic-matches a real HDFC "Sent Rs.X" UPI debit (a different HDFC message shape than the VPA-debit specific pattern)', () => {
    const result = parseSms(SMS_PATTERN_SEEDS, { sender: 'HDFCBK', body: 'Sent Rs.30.00\nFrom HDFC Bank A/C *0942\nTo Bengaluru Metro\nOn 05/09/26\nRef 214915313970\nNot You?\nCall 18002586161/SMS BLOCK UPI to 7308080808' })
    expect(result).not.toBeNull()
    expect(result.amount).toBe('30.00')
    expect(result.type).toBe('expense')
    expect(result.last4_hint).toBe('0942')
    expect(result.merchant).toBe('Bengaluru Metro')
  })
