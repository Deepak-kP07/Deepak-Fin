...
export const SMS_PATTERN_SEEDS = [
  ...
  {
    // Real observed format: "Dear Customer, Acct XXX601 Dr. INR 90.00 on 30/08/26 to PENTUMANI
    // VE; UPI: 128758271865; Bal INR 10,060.87...-CanaraBank" — no last4_hint here on purpose:
    // Canara only ever reveals the last 3 digits ("XXX601"), which can never exact-match
    // accounts.account_number_last4 (a real 4-digit value) — account resolution for this pattern
    // relies entirely on the bank-name fallback in genericCrud.js instead. sender_id_pattern is a
    // guess (this was seen forwarded via WhatsApp, which strips the real SMS sender header) — if
    // it doesn't match your real sender, this still works via the generic fallback, just less
    // precisely.
