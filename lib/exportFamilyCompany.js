...
export function buildFamilyCompanyExportCsv({ profiles, entries, categories = [] }) {
  ...
  const rows = entries.map((e) => {
    ...
    return [
      ...
      e.date, e.entry_type, categoryById(e.category_id)?.name || '', e.description, e.paid_party || '', Number(e.amount).toFixed(2), e.notes || '',
    ]
  })
