...
// Nudges a color's hue by a fixed offset — used to keep two entries that happen to land on the
// exact same stored color (e.g. an income-side and an expense-side category that were each
// independently defaulted/picked to the same swatch) visually distinct within one chart/legend,
// without touching either entity's own stored color anywhere else it's used. Floors saturation
// so a near-grayscale source (which a bare hue rotation would do nothing visible to) still comes
// out as a real, distinguishable color rather than another shade of the same gray.
export function rotateHue(hex, degrees) {
  const hsl = hexToHsl(hex)
  if (!hsl) return hex
  return hslToHex((hsl.h + degrees + 360) % 360, Math.max(hsl.s, 0.35), hsl.l)
}
