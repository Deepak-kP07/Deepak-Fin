...
const DATA_KEYS = [
  ...
  'pending_transactions', 'sms_parse_patterns', 'chit_funds', 'chit_fund_payments',
]
...
// v4: chit_funds/chit_fund_payments were added to DATA_KEYS after v1 had already shipped — same
// reasoning as v3 above. Missing this step (as this file initially did) doesn't break anything
// visible on a live/online load, since refresh()'s setData(snapshot) sets every key directly from
// the live fetch regardless of Dexie — but it silently meant these two tables never made it into
// the offline snapshot at all, so a cold app load between the live refresh landing and Dexie
// catching up (or an offline-first load) would show 0 chit funds until the network caught up.
db.version(4).stores({
  chit_funds: 'id',
  chit_fund_payments: 'id',
})
