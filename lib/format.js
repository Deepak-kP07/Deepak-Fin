...
// Display-only — capitalizes just the first character, leaving the rest of a free-typed string
// (description, reason, notes, …) exactly as entered. Never write the result back to the
// database or feed it into search/sort/export, which must keep matching what the user actually
// typed.
export const capitalizeFirst = (s) => (s ? s.charAt(0).toUpperCase() + s.slice(1) : s)
...
// For a Date built from local y/m/d components (e.g. `new Date(year, month, day)`), reads back
// its LOCAL date instead of `.toISOString().slice(0,10)` — which reinterprets in UTC and rolls
// back a day for any positive-offset zone (IST included) since local midnight is still "yesterday
// evening" in UTC.
export const dateToLocalISO = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
