      ...
      <div className="text-sm text-slate-300 sm:col-span-2">Type
        <div className="mt-2 grid grid-cols-3 gap-2">
          {[{ v: 'bank', l: 'Bank' }, { v: 'cash', l: 'Cash' }, { v: 'debit_card', l: 'Debit card' }].map((t) => (
            <button key={t.v} type="button" onClick={() => setForm({ ...form, type: t.v })} className={`rounded-xl border px-3 py-2.5 text-sm font-medium transition ${form.type === t.v ? 'border-accent-300/50 bg-accent-400/10 text-white' : 'border-white/10 text-slate-400 hover:bg-white/5'}`}>{t.l}</button>
          ))}
        </div>
      </div>
      <label className="text-sm text-slate-300 sm:col-span-2">Name
        <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder={isDebitCard ? 'HDFC Visa Debit' : isCash ? 'Cash in hand' : 'HDFC Salary'} />
      </label>
      {isDebitCard ? (
        <>
          <label className="text-sm text-slate-300 sm:col-span-2">Linked bank account
            <Select required value={form.linked_account_id} onChange={(e) => setForm({ ...form, linked_account_id: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-[#101621] px-3 py-3 text-white outline-none focus:border-accent-300/50">
          ...
          <label className="text-sm text-slate-300 sm:col-span-2">Card's last 4 digits <span className="text-xs text-slate-500">(optional)</span>
            <input maxLength={4} value={form.account_number_last4 || ''} onChange={(e) => setForm({ ...form, account_number_last4: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder="1234" />
          </label>
        </>
      ) : (
        <>
          {!isCash && (
            <>
              <label className="text-sm text-slate-300">Bank name
                <input value={form.bank_name || ''} onChange={(e) => setForm({ ...form, bank_name: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder="HDFC Bank" />
              </label>
          ...
          <label className="text-sm text-slate-300">Opening balance
            <input required type="number" step="0.01" value={form.opening_balance} onChange={(e) => setForm({ ...form, opening_balance: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" />
          </label>
        </>
      )}
...
export function AccountForm({ open, onClose, onSaved, editing, accounts = [], toast, mutate }) {
  ...
  const submitButton = <button disabled={busy} className="mt-6 w-full rounded-xl bg-gradient-to-r from-accent-300 to-blue-500 py-3.5 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update account' : 'Save account'}</button>
