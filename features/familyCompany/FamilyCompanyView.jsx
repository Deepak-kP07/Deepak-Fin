  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-2 text-xs uppercase tracking-widest text-accent-200/70 light:text-accent-700">Managed on their behalf</div>
          <h1 className="text-3xl font-semibold tracking-tight text-white light:text-slate-900">Family / Company</h1>
        </div>
      ...
      {profiles.length === 0 ? (
        ...
        <div className="grid gap-4 grid-cols-[repeat(auto-fit,minmax(260px,360px))]">
          {profiles.map((p) => {
            ...
            return (
              <div key={p.id} onClick={() => setSelectedProfileId(p.id)} className={`cursor-pointer rounded-2xl border p-5 transition ${isClosed ? 'border-white/5 light:border-black/5 bg-white/[.02] light:bg-black/[.02] hover:bg-white/[.035] hover:light:bg-black/[.025]' : 'border-white/10 light:border-black/10 bg-white/[.035] light:bg-black/[.025] hover:border-accent-300/30 hover:bg-white/[.05] hover:light:bg-black/[.035]'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {p.linked_account_id && <span className="flex shrink-0 items-center gap-1 rounded-full bg-accent-400/15 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-widest text-accent-200 light:text-accent-700"><Link2 size={9} />Linked</span>}
                      {isClosed && <span className="shrink-0 rounded-full bg-slate-500/20 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-widest text-slate-300 light:text-slate-700">Closed</span>}
                    </div>
