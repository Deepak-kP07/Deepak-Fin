...
import { CategoryEditor } from '@/features/familyCompany/CategoryEditor'

export function MoneyProfileEntryForm({ open, onClose, onSaved, editing, profile, onUpdateCategories, toast }) {
  const initial = editing
    ? { ...editing, amount: String(editing.amount) }
    : { profile_id: profile?.id, entry_type: 'expense', category: '', description: '', amount: '', date: todayISO(), paid_party: '', notes: '' }
  const [form, setForm] = useState(initial)
  const [busy, setBusy] = useState(false)
  const [manageCategories, setManageCategories] = useState(false)
  // Keyed on profile?.id, not the profile object itself — that object gets a fresh identity on
  // every refresh() (e.g. right after adding a category from the pencil-icon editor below), and
  // resetting the form on every unrelated data refresh would wipe out whatever the user had
  // already typed.
  useEffect(() => { setForm(initial); setManageCategories(false) }, [editing, open, profile?.id])
  if (!open) return null

  // An entry's saved category may no longer be in the profile's (customizable, editable-later)
  // category list — keep it selectable so editing the entry never silently discards it.
  const categoryOptions = form.category && !(profile?.categories || []).includes(form.category)
    ? [...(profile?.categories || []), form.category]
    : (profile?.categories || [])
  ...
  const save = async (e) => {
    ...
    try {
      ...
      const payload = { profile_id: profile.id, entry_type: form.entry_type, category: form.category || null, description: form.description, amount: Number(form.amount), date: form.date, paid_party: form.paid_party || null, notes: form.notes || null }
      ...
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-white/10 bg-[#141a28] p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <div className="text-sm text-slate-300">
            <div className="flex items-center justify-between">
              <label htmlFor="entry-category">Category <span className="text-xs text-slate-500">(optional)</span></label>
              <button type="button" onClick={() => setManageCategories((v) => !v)} className="flex items-center gap-1 text-xs text-cyan-300 hover:underline"><Pencil size={11} />Edit</button>
            </div>
            <Select id="entry-category" value={form.category || ''} onChange={(e) => setForm({ ...form, category: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-[#101621] px-3 py-3 text-white outline-none">
        ...
        {manageCategories && <div className="mt-4"><CategoryEditor profile={profile} onUpdateCategories={onUpdateCategories} /></div>}
