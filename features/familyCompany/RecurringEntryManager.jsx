...
import { ENTRY_TYPE_STYLE } from '@/lib/moneyProfiles'
import { capitalizeFirst, formatDate, money } from '@/lib/format'

export function RecurringEntryManager({ open, onClose, profile, rules, onAdd, onEdit, onToggle, onDelete, showMoney }) {
    ...
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/60 p-4 backdrop-blur-sm sm:items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-white/10 light:border-black/10 bg-[#141a28] light:bg-white p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-white light:text-slate-900">Recurring entries · {profile.name}</h2>
            <p className="mt-1 text-xs text-slate-500">Salary, rent, subscriptions — set once, logged automatically.</p>
          </div>
        ...
        <button type="button" onClick={onAdd} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-accent-300 to-accent-600 py-3 text-sm font-semibold text-[#07101c]"><Plus size={15} />New recurring entry</button>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-widest ${ENTRY_TYPE_STYLE[r.entry_type]}`}>{r.entry_type}</span>
                    <div className="truncate text-sm font-medium text-white light:text-slate-900">{capitalizeFirst(r.description)}</div>
                  </div>
                  <div className="mt-1 text-xs capitalize text-slate-500">{r.frequency} · {showMoney ? money(r.amount) : '••••'} · next {formatDate(r.next_due_date)}</div>
                </div>
