    ...
    <div className="rounded-2xl border border-white/10 bg-white/[.035] p-5">
      ...
      <div className="mt-4 flex gap-1.5 rounded-xl bg-black/20 p-1">
        {['expense', 'income'].map((t) => (
          <button key={t} onClick={() => setType(t)} className={`flex-1 rounded-lg py-2 text-sm font-medium capitalize transition ${type === t ? 'bg-cyan-400/15 text-cyan-200' : 'text-slate-400 hover:text-white'}`}>{t}</button>
        ))}
      </div>
                ...
                {MODULE_OPTIONS.map((m) => {
                  ...
                  return (
                    <td key={m.key} className="px-2 py-2 text-center">
                      <input type="checkbox" checked={!hidden} onChange={() => onToggleCategoryModule(c, m.key)} className="h-4 w-4 rounded border-white/20 bg-transparent accent-cyan-400" />
                    </td>
                  )
                })}
                <td className="px-2 py-2">
