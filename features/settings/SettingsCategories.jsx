    ...
    <div className="rounded-2xl border border-white/10 light:border-black/10 bg-[#0e121c] light:bg-black/[.025] glassy:glass-card p-5">
        ...
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              <tr key={c.id} className="border-t border-white/5 light:border-black/5">
                <td className="px-2 py-2">
                  <div className="flex items-center gap-2">
                    <div className="flex flex-col">
                      <button disabled={i === 0} onClick={() => onReorderCategory(c, rows[i - 1])} className="rounded p-0.5 text-slate-500 hover:text-white hover:light:text-slate-900 disabled:opacity-20"><ChevronUp size={12} /></button>
                      <button disabled={i === rows.length - 1} onClick={() => onReorderCategory(c, rows[i + 1])} className="rounded p-0.5 text-slate-500 hover:text-white hover:light:text-slate-900 disabled:opacity-20"><ChevronDown size={12} /></button>
                    </div>
      ...
      <div className="mt-4 space-y-2.5 sm:hidden">
        {rows.length === 0 ? (
          ...
          <div key={c.id} className="rounded-xl border border-white/10 light:border-black/10 bg-white/[.02] light:bg-black/[.02] p-3">
            <div className="flex items-center gap-2">
              <div className="flex flex-col">
                <button disabled={i === 0} onClick={() => onReorderCategory(c, rows[i - 1])} className="rounded p-0.5 text-slate-500 hover:text-white hover:light:text-slate-900 disabled:opacity-20"><ChevronUp size={12} /></button>
                <button disabled={i === rows.length - 1} onClick={() => onReorderCategory(c, rows[i + 1])} className="rounded p-0.5 text-slate-500 hover:text-white hover:light:text-slate-900 disabled:opacity-20"><ChevronDown size={12} /></button>
