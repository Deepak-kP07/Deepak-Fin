  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="mb-1 text-xs uppercase tracking-widest text-accent-200/70">Your space</div>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
        <div className="flex w-full min-w-0 gap-1.5 overflow-x-auto pb-1 lg:w-48 lg:shrink-0 lg:flex-col lg:overflow-visible lg:pb-0">
          {SECTIONS.map((s) => (
            <button
              key={s.key}
              onClick={() => onSectionChange(s.key)}
              className={`flex shrink-0 items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition lg:w-full ${active.key === s.key ? 'bg-accent-400/10 text-accent-200' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
            >
