    ...
    <div className="rounded-2xl border border-white/10 light:border-black/10 bg-white/[.035] light:bg-black/[.025] glassy:glass-card p-5">
      ...
      <div className="mt-3 space-y-1.5">
        {accounts.length === 0 ? <div className="text-sm text-slate-500">No accounts yet</div> : accounts.map((a, i) => (
          <div key={a.id} className="flex items-center gap-3 rounded-xl bg-black/20 light:bg-black/[.06] px-3 py-2">
            <div className="flex flex-col">
              <button disabled={i === 0} onClick={() => onReorderAccount(a, accounts[i - 1])} className="rounded p-0.5 text-slate-500 hover:text-white hover:light:text-slate-900 disabled:opacity-20"><ChevronUp size={14} /></button>
              <button disabled={i === accounts.length - 1} onClick={() => onReorderAccount(a, accounts[i + 1])} className="rounded p-0.5 text-slate-500 hover:text-white hover:light:text-slate-900 disabled:opacity-20"><ChevronDown size={14} /></button>
            </div>
            <Landmark size={14} className="text-slate-500" />
            <span className="text-sm text-white light:text-slate-900">{a.name}</span>
