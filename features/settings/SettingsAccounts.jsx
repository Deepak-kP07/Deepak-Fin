...
import { Eye, EyeOff, GripVertical, Landmark } from 'lucide-react'

function SortableAccountRow({ account, onToggleVisible }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: account.id })
  const style = { transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 10 : undefined }
  const hidden = account.is_active === false
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-3 rounded-xl bg-black/20 light:bg-black/[.06] px-3 py-2 ${isDragging ? 'opacity-60' : ''} ${hidden ? 'opacity-50' : ''}`}>
      ...
      <span className="min-w-0 flex-1 truncate text-sm text-white light:text-slate-900">{account.name}</span>
      <span className="shrink-0 text-xs capitalize text-slate-500">{account.type.replace('_', ' ')}</span>
      <button
        type="button"
        onClick={() => onToggleVisible(account)}
        title={hidden ? 'Hidden from forms — tap to show' : 'Hide from forms'}
        className="shrink-0 rounded-lg p-1.5 text-slate-500 hover:bg-white/5 hover:text-white hover:light:text-slate-900"
      >
...
export function SettingsAccounts({ data, onReorderAccounts, onToggleAccountVisible }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 light:border-black/10 bg-[#0e121c] light:bg-black/[.025] glassy:glass-card p-5">
      ...
      <div className="text-xs text-slate-500">Drag to reorder how accounts appear in every dropdown across the app. The eye icon hides an account from those dropdowns without touching its balance or history.</div>
      {accounts.length === 0 ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
          <SortableContext items={accounts.map((a) => a.id)} strategy={verticalListSortingStrategy}>
            <div className="mt-3 space-y-1.5">
              {accounts.map((a) => <SortableAccountRow key={a.id} account={a} onToggleVisible={onToggleAccountVisible} />)}
            </div>
          </SortableContext>
        </DndContext>
      )}
