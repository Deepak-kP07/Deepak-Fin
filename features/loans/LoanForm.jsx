    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm text-slate-300">Name
          <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder="Home loan" />
        </label>
        <label className="text-sm text-slate-300">Lender
          <input value={form.lender || ''} onChange={(e) => setForm({ ...form, lender: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder="HDFC Bank" />
        </label>
        <label className="text-sm text-slate-300">Principal
          <input required type="number" step="0.01" min="0" value={form.principal} onChange={(e) => setForm({ ...form, principal: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300">Interest rate % p.a.
          <input required type="number" step="0.01" min="0" value={form.interest_rate} onChange={(e) => setForm({ ...form, interest_rate: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300">Tenure (months)
          <input required type="number" min="1" value={form.tenure_months} onChange={(e) => setForm({ ...form, tenure_months: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300">EMI amount
          <div className="mt-2 flex gap-2">
            <input required type="number" step="0.01" min="0" value={form.emi_amount} onChange={(e) => setForm({ ...form, emi_amount: e.target.value })} className="w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" />
            <button type="button" onClick={suggestEmi} className="rounded-xl border border-white/10 px-3 text-xs text-accent-200 hover:bg-white/5">Calc</button>
          </div>
        </label>
        <label className="text-sm text-slate-300">Start date
          <DateInput value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300">EMI due day <span className="text-xs text-slate-500">(1–31, optional)</span>
          <input type="number" min="1" max="31" value={form.emi_due_day} onChange={(e) => setForm({ ...form, emi_due_day: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder="e.g. 5" />
        </label>
        ...
        <label className="text-sm text-slate-300">Current outstanding <span className="text-xs text-slate-500">(defaults to principal)</span>
          <input type="number" step="0.01" min="0" value={form.outstanding} onChange={(e) => setForm({ ...form, outstanding: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder={form.principal || '0.00'} />
        </label>
      </div>
      ...
      {!editing && (
        <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-4">
          ...
          <label className="mt-2 block text-xs text-slate-400">Months already paid
            <input type="number" min="0" max="60" value={form.monthsAlreadyPaid} onChange={(e) => setForm({ ...form, monthsAlreadyPaid: e.target.value })} className="mt-1 w-32 rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-white outline-none focus:border-accent-300/50" placeholder="0" />
          </label>
          {backfillSchedule?.length > 0 && (
            <div className="mt-3 rounded-lg bg-white/[.03] p-3 text-xs text-slate-400">
              ...
              <button type="button" onClick={applyBackfillOutstanding} className="mt-2 rounded-lg border border-accent-300/30 px-3 py-1.5 text-xs font-medium text-accent-200 hover:bg-accent-300/10">Use this as current outstanding</button>
...
export function LoanForm({ open, onClose, onSaved, editing, accounts, toast }) {
  ...
  const submitButton = <button disabled={busy} className="mt-6 w-full rounded-xl bg-gradient-to-r from-accent-300 to-blue-500 py-3.5 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update loan' : 'Save loan'}</button>
