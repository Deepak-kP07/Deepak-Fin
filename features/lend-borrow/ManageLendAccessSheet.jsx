            ...
            {shares.map((s) => {
              ...
              return (
                <div key={s.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 light:border-black/10 bg-white/[.02] light:bg-black/[.02] px-4 py-3">
                  ...
                  {isAccepted && canTouch ? (
                    <Select value={s.role} onChange={(e) => onRoleChange(s, e.target.value)} className="w-32 rounded-lg border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-2.5 py-2 text-xs text-white light:text-slate-900 outline-none">
