...
function LendFormFields({ form, setForm, accounts, creditCards }) {
  return (
    <>
...
// Entirely online-only, not split by create/edit branch — creating a record mirrors a
// transaction (and, if card-funded, bumps credit_cards.current_outstanding) server-side;
// editing has no such side effect, but one save() function handles both, so the whole form
// stays online-only rather than splitting behavior within a single function (same call as
// LoanForm).
export function LendForm({ open, onClose, onSaved, editing, accounts, creditCards = [], toast }) {
  ...
  const save = async (e) => {
    ...
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      toast.push('Logging a lend/borrow record needs a connection — try again once you’re back online.', 'error')
      return
    }
    setBusy(true)
    try {
  ...
  if (isMobile) {
    return (
      <BottomSheet open={open} onOpenChange={(v) => { if (!v) onClose() }} title={editing ? 'Edit record' : 'Log lend or borrow'}>
        <form onSubmit={save}>
