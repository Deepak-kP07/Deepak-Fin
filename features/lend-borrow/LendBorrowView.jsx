...
export function LendBorrowView({ data, onAdd, onEdit, onDelete, onDeleteTx, onLogRepayment, showMoney, onToggleMoney, toast }) {
    ...
    return (
      <div key={l.id} onClick={() => setSelectedId(l.id)} className={`cursor-pointer rounded-2xl border p-5 transition ${isClosed ? 'border-white/5 bg-white/[.02] hover:bg-white/[.035]' : isLent ? 'border-emerald-400/10 bg-emerald-500/[.03] hover:bg-white/[.02]' : 'border-rose-400/10 bg-rose-500/[.03] hover:bg-white/[.02]'}`}>
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] uppercase tracking-widest ${isClosed ? 'bg-slate-500/15 text-slate-400' : l.status === 'partial' ? 'bg-amber-400/15 text-amber-200' : 'bg-accent-400/15 text-accent-200'}`}>{l.status}</span>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-2 text-xs uppercase tracking-widest text-accent-200/70">Money between people</div>
          <h1 className="text-3xl font-semibold tracking-tight text-white">Lend &amp; borrow</h1>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowHistory((v) => !v)} className={`flex flex-1 items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold transition sm:flex-none ${showHistory ? 'border-accent-300/50 bg-accent-400/10 text-accent-200' : 'border-white/10 text-slate-400 hover:bg-white/5'}`}>
            <History size={15} />{showHistory ? 'Hide settled' : 'View all history'}
          </button>
          <button onClick={onAdd} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-accent-300 to-blue-500 px-4 py-2.5 text-sm font-semibold text-[#07101c] sm:flex-none"><Plus size={15} />Log</button>
      ...
      {lend_borrow.length > 0 && (
        <div className="rounded-3xl border border-white/10 bg-white/[.035] p-6">
          ...
          <div className="mt-1 text-[clamp(2rem,6vw,3rem)] font-semibold leading-[1.1] tracking-[-0.01em] text-white">{showMoney ? money(Math.abs(net)) : '••••••'}</div>
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-white/[.04] p-3.5">
              <div className="text-xs text-slate-400">Total lent (pending)</div>
              <div className="mt-1 text-lg font-semibold text-emerald-300">{showMoney ? money(lentPending) : '••••'}</div>
              <div className="text-[11px] text-slate-500">{lent.length} people</div>
            </div>
            <div className="rounded-2xl bg-white/[.04] p-3.5">
              <div className="text-xs text-slate-400">Total borrowed (pending)</div>
              <div className="mt-1 text-lg font-semibold text-rose-300">{showMoney ? money(borrowedPending) : '••••'}</div>
              <div className="text-[11px] text-slate-500">{borrowed.length} people</div>
            </div>
          </div>
        </div>
      )}
