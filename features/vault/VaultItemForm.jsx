...
export function VaultItemForm({ open, onClose, onSaved, editing, accounts = [], toast, defaultType = 'bank_account' }) {
  ...
  const inputClass = 'mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50'
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/60 p-4 backdrop-blur-sm sm:items-center" onClick={onClose}>
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-white/10 bg-[#141a28] p-6">
        ...
        {prefilling ? (
          <div className="py-10 text-center text-sm text-slate-500">Decrypting…</div>
        ) : (
        ...
        <button disabled={busy || prefilling} className="mt-6 w-full rounded-xl bg-gradient-to-r from-cyan-300 to-blue-500 py-3.5 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update item' : 'Save to vault'}</button>
      </form>
    </div>
  )
}
