  ...
  return (
    <div className="grid gap-3">
      <label className="text-sm text-slate-300 light:text-slate-700">Fund name
        <input required value={form.fund_name} onChange={(e) => setForm({ ...form, fund_name: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder="Parag Parikh Flexi Cap" />
      </label>
      <div className="grid grid-cols-2 gap-3">
        <label className="text-sm text-slate-300 light:text-slate-700">Folio number <span className="text-xs text-slate-500">(optional)</span>
          <input value={form.folio_number || ''} onChange={(e) => setForm({ ...form, folio_number: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300 light:text-slate-700">Start date
          <DateInput value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
        </label>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <label className="text-sm text-slate-300 light:text-slate-700">Monthly amount
          <input required type="number" step="0.01" min="0" value={form.monthly_amount} onChange={(e) => setForm({ ...form, monthly_amount: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300 light:text-slate-700">Units held
          <input required type="number" step="0.0001" min="0" value={form.units_held} onChange={(e) => setForm({ ...form, units_held: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
        </label>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <label className="text-sm text-slate-300 light:text-slate-700">Latest NAV
          <input required type="number" step="0.0001" min="0" value={form.nav} onChange={(e) => setForm({ ...form, nav: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
        </label>
        <label className="text-sm text-slate-300 light:text-slate-700">Average buy NAV <span className="text-xs text-slate-500">(optional)</span>
          <input type="number" step="0.0001" min="0" value={form.average_price} onChange={(e) => setForm({ ...form, average_price: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder="Leave blank if unknown" />
        </label>
      </div>
      <label className="text-sm text-slate-300 light:text-slate-700">Portfolio <span className="text-xs text-slate-500">(optional — just a grouping label)</span>
        <Select value={form.portfolio_id} onChange={(e) => setForm({ ...form, portfolio_id: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-3 py-2.5 text-white light:text-slate-900 outline-none">
          <option value="">None</option>
          {portfolios.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
        </Select>
      </label>
      <div className="rounded-xl border border-white/10 light:border-black/10 bg-white/[.02] light:bg-black/[.02] px-3 py-2 text-xs text-slate-400 light:text-slate-500">Current value (units × NAV): <span className="font-semibold text-white light:text-slate-900">{money2(currentValue)}</span></div>
    </div>
  )
}
...
export function SipForm({ open, onClose, onSaved, editing, portfolios = [], toast, mutate }) {
  ...
  const submitButton = <button disabled={busy} className="mt-4 w-full rounded-xl bg-gradient-to-r from-accent-300 to-accent-600 py-3 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update SIP' : 'Save SIP'}</button>
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl border border-white/10 light:border-black/10 bg-[#141a28] light:bg-white p-5 max-h-[90vh] overflow-y-auto">
