  ...
  return (
    <div className="grid gap-4">
      <label className="text-sm text-slate-300">Amount
        <input required type="number" step="0.01" min="0.01" max={portfolio.cash_balance || undefined} value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-accent-300/50" placeholder="1000" />
      </label>
...
export function WithdrawFundsForm({ open, onClose, onSaved, portfolio, accounts, toast }) {
  ...
  const submitButton = <button disabled={busy} className="mt-6 w-full rounded-xl bg-gradient-to-r from-accent-300 to-blue-500 py-3.5 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Withdrawing…' : 'Withdraw funds'}</button>
