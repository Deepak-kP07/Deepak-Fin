...
// A custom action (POST .../add_funds), not plain table CRUD — requires connectivity, so this
// deliberately doesn't go through mutate()/the offline outbox (see Phase 5's design notes).
export function PortfolioFundsForm({ open, onClose, onSaved, portfolio, accounts, toast }) {
  ...
  const submitButton = <button disabled={busy} className="mt-6 w-full rounded-xl bg-gradient-to-r from-accent-300 to-accent-600 py-3.5 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Adding…' : 'Add funds'}</button>

  if (isMobile) {
    return (
      <BottomSheet open={open} onOpenChange={(v) => { if (!v) onClose() }} title="Add funds">
        <p className="-mt-2 mb-4 text-xs text-slate-500">{portfolio.name} · current cash {money(portfolio.cash_balance || 0)}</p>
        <form onSubmit={save}>
          <PortfolioFundsFormFields form={form} setForm={setForm} accounts={accounts} />
          {submitButton}
        </form>
      </BottomSheet>
    )
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl border border-white/10 light:border-black/10 bg-[#141a28] light:bg-white p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-white light:text-slate-900">Add funds</h2>
