  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="text-sm text-slate-300 light:text-slate-700 sm:col-span-2">Name
        <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder={isBond ? 'HDFC NCD 2030, RBI Floating Rate Bond…' : 'Gold necklace, 2 acre plot in Kolar…'} />
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700 sm:col-span-2">Category
        <Select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-3 py-2.5 text-white light:text-slate-900 outline-none">
      ...
      <label className="text-sm text-slate-300 light:text-slate-700">Purchase value
        <input required type="number" step="0.01" min="0" value={form.purchase_value} onChange={(e) => setForm({ ...form, purchase_value: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700">Purchase date
        <DateInput value={form.purchase_date} onChange={(e) => setForm({ ...form, purchase_date: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
      </label>
      ...
      {isBond ? (
        <>
          <label className="text-sm text-slate-300 light:text-slate-700">Face value <span className="text-xs text-slate-500">(paid at maturity)</span>
            <input required type="number" step="0.01" min="0" value={form.face_value} onChange={(e) => setForm({ ...form, face_value: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
          </label>
          <label className="text-sm text-slate-300 light:text-slate-700">Maturity date
            <DateInput value={form.maturity_date} onChange={(e) => setForm({ ...form, maturity_date: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
          </label>
          <label className="text-sm text-slate-300 light:text-slate-700">Coupon rate <span className="text-xs text-slate-500">(% / year, optional)</span>
            <input type="number" step="0.01" min="0" value={form.coupon_rate_pct} onChange={(e) => setForm({ ...form, coupon_rate_pct: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
          </label>
          <label className="text-sm text-slate-300 light:text-slate-700">Interest payout
            <Select value={form.interest_frequency} onChange={(e) => setForm({ ...form, interest_frequency: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-3 py-2.5 text-white light:text-slate-900 outline-none">
              {BOND_INTEREST_FREQUENCIES.map((f) => <option key={f.value} value={f.value}>{f.label}</option>)}
            </Select>
          </label>
          <div className="rounded-xl border border-white/10 light:border-black/10 bg-white/[.02] light:bg-black/[.02] px-3 py-2 text-xs text-slate-400 light:text-slate-500 sm:col-span-2">
        ...
        <>
          <label className="text-sm text-slate-300 light:text-slate-700">Expected CAGR <span className="text-xs text-slate-500">(% / year)</span>
            <input required type="number" step="0.1" value={form.expected_cagr_pct} onChange={(e) => setForm({ ...form, expected_cagr_pct: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
          </label>
          <div className="rounded-xl border border-white/10 light:border-black/10 bg-white/[.02] light:bg-black/[.02] px-3 py-2 text-xs text-slate-400 light:text-slate-500 sm:col-span-2">
          ...
          <label className="text-sm text-slate-300 light:text-slate-700">Last known value <span className="text-xs text-slate-500">(optional — a real revaluation)</span>
            <input type="number" step="0.01" min="0" value={form.last_known_value} onChange={(e) => setForm({ ...form, last_known_value: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder="Leave blank to project from purchase" />
          </label>
          <label className="text-sm text-slate-300 light:text-slate-700">As of <span className="text-xs text-slate-500">(defaults to today)</span>
            <DateInput value={form.last_known_value_date} onChange={(e) => setForm({ ...form, last_known_value_date: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
          </label>
        </>
      )}
      ...
      <label className="text-sm text-slate-300 light:text-slate-700 sm:col-span-2">Notes <span className="text-xs text-slate-500">(optional)</span>
        <input value={form.notes || ''} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
      </label>
    </div>
  )
}
...
export function OtherInvestmentForm({ open, onClose, onSaved, editing, portfolioId, toast, mutate }) {
  ...
  const submitButton = <button disabled={busy} className="mt-4 w-full rounded-xl bg-gradient-to-r from-accent-300 to-accent-600 py-3 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update investment' : 'Save investment'}</button>
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-xl rounded-3xl border border-white/10 light:border-black/10 bg-[#141a28] light:bg-white p-5 max-h-[90vh] overflow-y-auto">
