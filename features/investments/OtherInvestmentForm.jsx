...
export function OtherInvestmentForm({ open, onClose, onSaved, editing, portfolioId, toast }) {
  ...
  const save = async (e) => {
    ...
    try {
      const endpoint = editing ? `/api/finance/other_investments/${editing.id}` : '/api/finance/other_investments'
      const payload = {
      ...
      toast.push(editing ? 'Investment updated' : 'Investment added'); onSaved()
      ...
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-xl rounded-3xl border border-white/10 bg-[#141a28] p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <label className="text-sm text-slate-300 sm:col-span-2">Name
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" placeholder={isBond ? 'HDFC NCD 2030, RBI Floating Rate Bond…' : 'Gold necklace, 2 acre plot in Kolar…'} />
          </label>
          ...
          <label className="text-sm text-slate-300">Purchase value
            <input required type="number" step="0.01" min="0" value={form.purchase_value} onChange={(e) => setForm({ ...form, purchase_value: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
          </label>
          <label className="text-sm text-slate-300">Purchase date
            <DateInput value={form.purchase_date} onChange={(e) => setForm({ ...form, purchase_date: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
          </label>

          {isBond ? (
            <>
              <label className="text-sm text-slate-300">Face value <span className="text-xs text-slate-500">(paid at maturity)</span>
                <input required type="number" step="0.01" min="0" value={form.face_value} onChange={(e) => setForm({ ...form, face_value: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
              </label>
              <label className="text-sm text-slate-300">Maturity date
                <DateInput value={form.maturity_date} onChange={(e) => setForm({ ...form, maturity_date: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
              </label>
              <label className="text-sm text-slate-300">Coupon rate <span className="text-xs text-slate-500">(% / year, optional)</span>
                <input type="number" step="0.01" min="0" value={form.coupon_rate_pct} onChange={(e) => setForm({ ...form, coupon_rate_pct: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
              </label>
            ...
            <>
              <label className="text-sm text-slate-300">Expected CAGR <span className="text-xs text-slate-500">(% / year)</span>
                <input required type="number" step="0.1" value={form.expected_cagr_pct} onChange={(e) => setForm({ ...form, expected_cagr_pct: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
              </label>
              <div className="rounded-xl border border-white/10 bg-white/[.02] px-3 py-2.5 text-xs text-slate-400 sm:col-span-2">
              ...
              <label className="text-sm text-slate-300">Last known value <span className="text-xs text-slate-500">(optional — a real revaluation)</span>
                <input type="number" step="0.01" min="0" value={form.last_known_value} onChange={(e) => setForm({ ...form, last_known_value: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" placeholder="Leave blank to project from purchase" />
              </label>
              <label className="text-sm text-slate-300">As of <span className="text-xs text-slate-500">(defaults to today)</span>
                <DateInput value={form.last_known_value_date} onChange={(e) => setForm({ ...form, last_known_value_date: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" />
              </label>
            </>
          )}
        ...
        <button disabled={busy} className="mt-6 w-full rounded-xl bg-gradient-to-r from-cyan-300 to-blue-500 py-3.5 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update investment' : 'Save investment'}</button>
      </form>
    </div>
  )
}
