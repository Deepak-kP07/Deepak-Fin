  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="text-sm text-slate-300 light:text-slate-700 sm:col-span-2">Portfolio
        <Select required value={form.portfolio_id} onChange={(e) => setForm({ ...form, portfolio_id: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-3 py-2.5 text-white light:text-slate-900 outline-none">
      ...
      <label className="text-sm text-slate-300 light:text-slate-700">Symbol
        <input required value={form.symbol} onChange={(e) => setForm({ ...form, symbol: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 uppercase text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder="RELIANCE" />
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700">Exchange
        <Select value={form.exchange} onChange={(e) => setForm({ ...form, exchange: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-3 py-2.5 text-white light:text-slate-900 outline-none">
          <option value="NSE">NSE</option><option value="BSE">BSE</option>
        </Select>
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700">Asset type
        <Select value={form.asset_type || 'equity'} onChange={(e) => setForm({ ...form, asset_type: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-[#101621] light:bg-white px-3 py-2.5 text-white light:text-slate-900 outline-none">
          <option value="equity">Equity</option>
          <option value="gold">Gold</option>
        </Select>
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700">Company name <span className="text-xs text-slate-500">(optional)</span>
        <input value={form.company_name || ''} onChange={(e) => setForm({ ...form, company_name: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder="Reliance Industries" />
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700">Quantity
        <input required type="number" step="0.0001" min="0" value={form.qty} onChange={(e) => setForm({ ...form, qty: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700">Avg buy price
        <input required type="number" step="0.01" min="0" value={form.avg_buy_price} onChange={(e) => setForm({ ...form, avg_buy_price: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" />
      </label>
      <label className="text-sm text-slate-300 light:text-slate-700 sm:col-span-2">Current price <span className="text-xs text-slate-500">(starting value — refresh live prices anytime from the toolbar)</span>
        <input type="number" step="0.01" min="0" value={form.current_price} onChange={(e) => setForm({ ...form, current_price: e.target.value })} className="mt-1.5 w-full rounded-xl border border-white/10 light:border-black/10 bg-white/[.04] light:bg-black/[.03] px-3 py-2.5 text-white light:text-slate-900 outline-none focus:border-accent-300/50" placeholder="Same as avg if blank" />
      </label>
    </div>
  )
}
...
export function HoldingForm({ open, onClose, onSaved, editing, portfolios, defaultPortfolioId, profile, toast, mutate }) {
  ...
  const submitButton = <button disabled={busy} className="mt-4 w-full rounded-xl bg-gradient-to-r from-accent-300 to-accent-600 py-3 text-sm font-semibold text-[#07101c] disabled:opacity-60">{busy ? 'Saving…' : editing ? 'Update holding' : 'Save holding'}</button>
