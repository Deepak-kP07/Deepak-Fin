  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 sm:hidden">
        <button onClick={onAddPortfolio} className="flex-1 rounded-xl border border-white/10 px-4 py-2.5 text-sm text-slate-300 hover:bg-white/5">New portfolio</button>
      </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[.035] p-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div className="rounded-2xl bg-white/[.04] p-3.5">
            <div className="flex items-center gap-1.5 text-xs text-slate-400"><Target size={13} />Invested</div>
            <div className="mt-1 text-lg font-semibold text-white">{showMoney ? money(totalInvested) : '••••'}</div>
          </div>
          <div className={`rounded-2xl bg-white/[.04] p-3.5 ${totalPnl >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
            <div className="flex items-center gap-1.5 text-xs opacity-80"><TrendingUp size={13} />Overall P&amp;L</div>
            <div className="mt-1 text-lg font-semibold">{showMoney ? (totalPnl >= 0 ? '+' : '−') + money(totalPnl).replace('-', '') : '••••'}</div>
            <div className="text-[11px] opacity-70">{totalPnlPct >= 0 ? '+' : ''}{totalPnlPct.toFixed(2)}%</div>
          </div>
        </div>
      </div>
      ...
      <DismissibleBanner tone="cyan">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(!kiteConnected || kiteBroken) && (
            <button onClick={onConnectKite} className={`ml-auto rounded-lg px-3 py-1 text-[11px] font-semibold ${kiteBroken ? 'bg-amber-400/20 text-amber-100 hover:bg-amber-400/30' : 'bg-accent-300/20 text-accent-100 hover:bg-accent-300/30'}`}>{kiteBroken ? 'Reconnect Kite' : 'Connect Kite'}</button>
          )}
        </div>
        {kiteConnected && !anyPortfolioLinked && (
          <div className="mt-2.5 flex flex-wrap items-center gap-2 border-t border-accent-300/10 pt-2.5">
            {emptyPortfolios.length === 0 ? (
              <span className="text-accent-200/70">Create an empty portfolio first, then link it to Kite to sync your real holdings.</span>
            ) : (
              <>
                <span className="text-accent-200/70">Sync your real Zerodha holdings into:</span>
                <Select value={linkChoice} onChange={(e) => setLinkChoice(e.target.value)} className="min-w-[180px] rounded-lg border border-accent-300/20 bg-[#0b1420] px-2.5 py-1.5 text-[11px] text-accent-100 outline-none">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg bg-accent-300/20 px-3 py-1.5 text-[11px] font-semibold text-accent-100 hover:bg-accent-300/30 disabled:opacity-50"
      ...
      {portfolios.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {portfolios.map((p) => {
            ...
            return (
              <div key={p.id} onClick={() => setSelectedPortfolioId(p.id)} className="cursor-pointer rounded-2xl border border-white/10 bg-white/[.035] p-5 transition hover:border-accent-300/30 hover:bg-white/[.05]">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {p.kite_linked && <span className="shrink-0 rounded-full bg-accent-400/15 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-widest text-accent-200">Kite</span>}
                    </div>
      ...
      {kiteConnected && (
        <div className="-mt-3 text-xs text-accent-200/70">
