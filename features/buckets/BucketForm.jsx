...
export function BucketForm({ open, onClose, onSaved, editing, toast }) {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      toast.push(editing ? 'Updated' : 'Added — the 30-day clock starts now'); onSaved()
      ...
      <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl border border-white/10 bg-[#141a28] p-6">
        ...
        <div className="mt-5 grid gap-4">
          <label className="text-sm text-slate-300">Product
            <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" placeholder="iPhone 15 Pro" />
          </label>
          <label className="text-sm text-slate-300">Product link <span className="text-xs text-slate-500">(optional)</span>
            <input type="url" value={form.product_url} onChange={(e) => setForm({ ...form, product_url: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" placeholder="https://amazon.in/…" />
          </label>
          <label className="text-sm text-slate-300">Estimated cost <span className="text-xs text-slate-500">(optional)</span>
            <input type="number" step="0.01" min="0" value={form.estimated_cost} onChange={(e) => setForm({ ...form, estimated_cost: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" placeholder="60000" />
          </label>
          <div className="text-sm text-slate-300">Reasons to buy <span className="text-xs text-slate-500">(up to 3)</span>
            <div className="mt-2 space-y-2">
              {[0, 1, 2].map((i) => (
                <input key={i} value={form.reasons[i]} onChange={(e) => setReason(i, e.target.value)} className="w-full rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-white outline-none focus:border-cyan-300/50" placeholder={`Reason ${i + 1}`} />
              ))}
            </div>
          </div>
        </div>
