...
export function BucketListView({ data, onAdd, onEdit, onDelete, showMoney, onToggleMoney }) {
  ...
  const readyValue = bucket_list.filter((b) => daysSince(b.created_at) >= WAIT_DAYS).reduce((s, b) => s + Number(b.estimated_cost || 0), 0)
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-2 text-xs uppercase tracking-widest text-accent-200/70 light:text-accent-700">The 30-day rule</div>
      ...
      {bucket_list.length > 0 && (
        <div className="rounded-3xl border border-white/10 light:border-black/10 bg-[#141a28] light:bg-black/[.025] glassy:glass-card p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <HeroStatTile
              label="Ready to buy value"
              value={showMoney ? money(readyValue) : '••••'}
              valueTone="text-emerald-300 light:text-emerald-700"
              sub={`${readyCount} item${readyCount === 1 ? '' : 's'}`}
            />
            <HeroStatTile
              label="Still waiting"
              value={showMoney ? money(stillWaitingValue) : '••••'}
              sub={`${bucket_list.length - readyCount} item${bucket_list.length - readyCount === 1 ? '' : 's'}`}
            />
          </div>
        </div>
      )}
            ...
            {bucket_list.map((b) => {
              ...
              return (
                <div key={b.id} className="grid grid-cols-1 gap-3 px-5 py-4 sm:grid-cols-[1.6fr_1fr_.7fr_auto] sm:items-center sm:gap-4">
                  ...
                  <div>
                    <div className={`flex items-center gap-1.5 text-sm font-semibold ${ready ? 'text-emerald-300 light:text-emerald-700' : 'text-white light:text-slate-900'}`}>
                    ...
                    {ready && <div className="mt-1 text-[11px] text-emerald-300/80 light:text-emerald-700">30 days up — still want it?</div>}
                  </div>

                  <div className="flex justify-end gap-1">
                    <button onClick={() => onEdit(b)} className="rounded-lg p-1.5 text-slate-500 hover:bg-white/5 hover:text-white hover:light:text-slate-900"><Pencil size={14} /></button>
                    <button onClick={() => onDelete(b)} className="rounded-lg p-1.5 text-rose-300/70 light:text-rose-700 hover:bg-rose-300/10"><Trash2 size={14} /></button>
                  </div>
                </div>
              )
            })}
