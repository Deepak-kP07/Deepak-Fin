  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between">
        <div>
          <div className="mb-2 text-xs uppercase tracking-widest text-cyan-200/70">The 30-day rule</div>
              ...
              return (
                <div key={b.id} className="grid grid-cols-1 gap-3 px-5 py-4 sm:grid-cols-[1.6fr_1fr_.7fr_auto] sm:items-center sm:gap-4">
                  <div className="min-w-0">
                    {b.product_url ? (
                      <a href={b.product_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 truncate text-sm font-medium text-cyan-200 hover:underline">
                        <span className="truncate">{b.title}</span>
                        <ExternalLink size={12} className="shrink-0 text-cyan-300/70" />
                      </a>
                    ) : (
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 h-1.5 w-24 overflow-hidden rounded-full bg-white/5"><div className={`h-full rounded-full ${ready ? 'bg-emerald-400' : 'bg-cyan-300'}`} style={{ width: `${pct}%` }} /></div>
