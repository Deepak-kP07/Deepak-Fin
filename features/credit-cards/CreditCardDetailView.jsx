...
export function CreditCardDetailView({ card, cardTransactions, allTransactions, categories, onBack, onSpend, onPay, onDeleteSpend, onDeleteTx, onEdit, onDelete, showMoney, onToggleMoney }) {
  ...
  return (
    <div className="space-y-5">
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-white"><ChevronRight size={14} className="rotate-180" /> Back to credit cards</button>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Outstanding" value={showMoney ? money(card.current_outstanding) : '••••'} icon={ArrowDownRight} accent="bg-rose-400/15 text-rose-200" tone="text-rose-300" sub={<span>of {money(card.credit_limit)} limit</span>} />
        <StatCard label="Utilisation" value={`${util}%`} icon={Target} accent="bg-violet-400/15 text-violet-200" tone={util <= 30 ? 'text-emerald-300' : util <= 60 ? 'text-amber-300' : 'text-rose-300'} sub={<span>{util <= 30 ? 'Healthy' : util <= 60 ? 'Rising' : 'High'}</span>} />
        <StatCard label="Total repaid" value={showMoney ? money(totalRepaid) : '••••'} icon={ArrowUpRight} accent="bg-emerald-400/15 text-emerald-200" sub={<span>{repayments.length} payment{repayments.length === 1 ? '' : 's'}</span>} />
      </div>

      <div className="rounded-2xl border border-white/10 bg-white/[.035] p-5">
        <div className="text-sm font-semibold text-white">Net spend by month · last 6 months</div>
                ...
                <div key={`${a.source}-${a.id}`} className="grid grid-cols-1 gap-2 px-5 py-4 sm:grid-cols-[1.4fr_.9fr_.6fr_.6fr_auto] sm:items-center sm:gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-white">{a.description}</div>
                      {a.status && <div className="text-[11px] uppercase tracking-widest text-slate-500">{a.status}</div>}
                    </div>
                  </div>
                  <div className="text-xs text-slate-400">
                    <span className="inline-block rounded-md bg-white/[.05] px-2 py-0.5" style={{ color: cat?.color || '#94a3b8' }}>{cat?.name || 'Uncategorised'}</span>
                  </div>
                  <div className="text-xs text-slate-500">{formatDateTime(a.date, a.time)}</div>
                  <div className={`text-sm font-semibold sm:text-right ${isDebit ? 'text-rose-300' : 'text-emerald-300'}`}>{isDebit ? '-' : '+'}{showMoney ? money(a.amount) : '••••'}</div>
                  <div className="flex justify-end">
                    <button onClick={() => deleteActivity(a)} className="rounded-lg p-1.5 text-rose-300/70 hover:bg-rose-300/10"><Trash2 size={13} /></button>
                  </div>
                </div>
              )
            })}
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[.035]">
        <div className="border-b border-white/10 px-5 py-3 text-xs uppercase tracking-widest text-slate-500">Repayment history · {repayments.length}</div>
        {repayments.length === 0 ? (
          ...
          <div className="max-h-64 divide-y divide-white/5 overflow-y-auto">
            {repayments.map((r) => (
              <div key={r.id} className="flex items-center justify-between px-5 py-3 text-sm">
                <div className="text-slate-300">{formatDate(r.date)}</div>
                <div className="font-medium text-emerald-300">{showMoney ? `+${money(r.amount)}` : '••••'}</div>
              </div>
            ))}
          </div>
        )}
