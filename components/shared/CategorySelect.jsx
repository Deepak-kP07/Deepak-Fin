      ...
      {open && (
        <div className="absolute left-0 z-30 mt-1 max-h-80 w-max min-w-full max-w-[min(24rem,90vw)] overflow-y-auto rounded-xl border border-white/10 bg-[#141a28] p-3 shadow-2xl">
          <div className="mb-2 flex items-center justify-between">
            ...
            {onAddCategory && (
              <button type="button" onClick={() => { setOpen(false); onAddCategory() }} className="rounded-lg p-1 text-slate-400 hover:bg-white/5 hover:text-accent-200" title="Add new category"><Pencil size={13} /></button>
            )}
          </div>
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
            <button type="button" onClick={() => { onChange({ target: { value: '' } }); setOpen(false) }} className={`rounded-lg px-3 py-2 text-left text-sm transition ${!value ? 'bg-accent-400/15 text-accent-200' : 'text-slate-300 hover:bg-white/5'}`}>{placeholder}</button>
            {categories.map((c) => (
              <button key={c.id} type="button" onClick={() => { onChange({ target: { value: c.id } }); setOpen(false) }} className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-left text-sm transition ${String(value) === String(c.id) ? 'bg-accent-400/15 text-accent-200' : 'text-slate-300 hover:bg-white/5'}`}>
