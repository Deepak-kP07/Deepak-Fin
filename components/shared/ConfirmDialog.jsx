  ...
  const view = state && (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => close(false)}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-3xl border border-white/10 bg-[#141a28] p-6 shadow-2xl">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-rose-400/15 text-rose-300">{state.okOnly ? <Ban size={18} /> : <Trash2 size={18} />}</div>
        <p className="mt-4 text-sm text-slate-200">{state.message}</p>
        <div className="mt-6 flex justify-end gap-2">
          {!state.okOnly && <button type="button" onClick={() => close(false)} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm text-slate-300 hover:bg-white/5">Cancel</button>}
          <button type="button" onClick={() => close(state.okOnly ? false : true)} className="rounded-xl bg-gradient-to-r from-rose-400 to-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90">{state.confirmLabel}</button>
        </div>
      </div>
    </div>
  )
