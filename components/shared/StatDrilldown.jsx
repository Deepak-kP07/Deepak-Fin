...
// than an action.
export function StatDrilldown({ open, onClose, title, icon: Icon, accent = 'bg-accent-400/15 text-accent-200 light:text-accent-700', rows = [], note, body }) {
  if (!open) return null
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-3xl border border-white/10 light:border-black/10 bg-[#141a28] light:bg-white p-6 shadow-2xl glassy:glass-card">
        ...
        {/* body: a plain explanatory paragraph — for a pure "what does this mean" tooltip with no
            line items. rows: the Income/Expense/Savings-style itemized breakdown; only rendered
            when there's actually something to list, so a rows-less drilldown doesn't leave an
            empty divider block sitting between the title and its note. */}
        {body && <p className="mt-3 text-sm leading-5 text-slate-300 light:text-slate-700">{body}</p>}
        {rows.length > 0 && (
