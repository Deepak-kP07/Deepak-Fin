...
export function MonthCursor({ cursor, onShift, showAll, onToggleAll }) {
  return (
    <div className="flex items-center gap-1">
      <div className={`flex items-center rounded-xl border border-white/10 transition-opacity ${showAll ? 'opacity-50' : ''}`}>
        <button type="button" onClick={() => onShift(-1)} className="rounded-l-xl p-2 text-slate-400 hover:bg-white/5 hover:text-white" title="Previous month"><ChevronLeft size={14} /></button>
        <span className="w-9 text-center text-xs font-semibold uppercase tracking-wider text-slate-300" title={`${MONTH_NAMES[cursor.month]} ${cursor.year}`}>{MONTH_NAMES[cursor.month].slice(0, 3)}</span>
        <button type="button" onClick={() => onShift(1)} className="rounded-r-xl p-2 text-slate-400 hover:bg-white/5 hover:text-white" title="Next month"><ChevronRight size={14} /></button>
      </div>
      <button
