...
Every surface color is translucent white over one of two near-black bases; no light-mode ground exists or is planned.
...
- **Primary:** `Glass Fill Strong` background, `Text Primary` text; the cyan accent is reserved for the hover/active state (`bg-accent`, dark text) rather than the resting state — this keeps cyan feeling earned rather than ambient.
...
### Navigation
- **Mobile:** bottom tab bar, already implemented; the active tab is the one place besides primary buttons where the cyan accent should appear at full strength.
- **Desktop:** sidebar, mirroring the same items and active-state treatment as the mobile bottom nav.

### Loading State (planned — not yet built)
- Framer Motion is an installed but currently unused dependency; the confirmed direction is a **pulse animation** on loading skeletons/placeholders (a `Glass Fill` block breathing in opacity) rather than a spinner, matching the vault's "settle, don't announce" character. Build this as the standard loading treatment wherever data is fetched, not a one-off per screen.

## Do's and Don'ts

### Do:
- **Do** treat cyan as the app's only accent color everywhere — including any future module built from scratch. No module gets its own brand hue.
...
- **Don't** push the accent glow (`shadow-cyan`) onto more than a small, deliberate set of elements — its rarity is what makes it register as premium rather than gimmicky.
